...
# --- Validación XSD (cache de esquema) ---
_XSD_SCHEMA = None
def _get_schema():
    global _XSD_SCHEMA
...
def validate_xml_against_xsd(xml_bytes: bytes) -> None:
    """Lanza excepción si el XML no cumple el XSD."""
    try:
        schema = _get_schema()
        schema.validate(xml_bytes)  # admite bytes/str/etree
    except Exception as e:
        raise ValueError(f"Validación XSD fallida: {e}")
...
def create_xml_from_data(df_factura_single, df_conceptos_single, df_forma_pago_single, df_conceptos_texto_single=None, df_factura_all=None, df_conceptos_all=None):
    ...
    for _, factura_row in df_factura_single.iterrows():
        ...
        if tipo_factura_norm.startswith("R"):
            ...
            if pd.notna(factura_row.get("factura_rectificativa_codigo", None)):
                create_sub_element(proforma, "factura_rectificativa_codigo", factura_row.get("factura_rectificativa_codigo"))
